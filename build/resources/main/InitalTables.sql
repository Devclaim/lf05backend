...
CREATE FUNCTION DELETEADRESSE() RETURNS TRIGGER AS
$$
BEGIN
    DELETE
    FROM ADRESSE
    WHERE ADRESSE.ADRESSID = (SELECT ADRESSID
                              FROM KUNDE
                              WHERE KUNDE.KUNDENNR = OLD.KUNDENNR);
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER deleteAdresseWhenKundeRowDrops
    BEFORE DELETE
    ON KUNDE
    FOR ROW
EXECUTE PROCEDURE DELETEADRESSE();

-- FUNCTIONS

CREATE FUNCTION GetRezepteByZutatEigenschaft(IN eigenschaftname VARCHAR(50)) RETURNS SETOF REZEPT
    LANGUAGE plpgsql
AS
$BODY$
    DECLARE
    ...
    BEGIN
        --CREATE VIEW TempLegalZutat AS SELECT * FROM ZUTATEIGENSCHAFT WHERE ZUTATEIGENSCHAFT.EIGENSCHAFTID =
        --    (SELECT EIGENSCHAFTID FROM EIGENSCHAFTEN WHERE BEZEICHNUNG = eigenschaftname);
        CREATE TABLE RESULTS AS SELECT * FROM REZEPT;
